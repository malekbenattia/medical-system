...
export function InsightsReports({ patients = [] }) {
  const [searchTerm, setSearchTerm] = useState("")
  const [selectedNote, setSelectedNote] = useState(null)

  const [medicalNotes, setMedicalNotes] = useState([])

  // Fetch structured SOAP notes from backend
  useEffect(() => {
  ...
  const filteredNotes = medicalNotes
    .map((note) => ({
      ...note,
      patientName: note.full_name || patientNameMap[note.patientId]  ,
    }))
    .filter(
      (note) =>
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredNotes.length === 0 ? (
          ...
          filteredNotes.map((note) => (
            <Card key={note.patient_ID} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setSelectedNote(selectedNote === note.patient_ID ? null : note.patient_ID)}
                    >
                      <Eye className="w-4 h-4 mr-2" />
                      {selectedNote === note.patient_ID ? "Hide Details" : "View Details"}
                    </Button>
                  </div>
                </div>
              </CardHeader>

              {selectedNote === note.patient_ID && (
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <Card className="bg-blue-50 border-blue-200">
                      ...
                      <CardContent>
                        <p className="text-sm text-blue-900">{note.subjective}</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="bg-green-50 border-green-200">
                      ...
                      <CardContent>
                        <p className="text-sm text-green-900">{note.objective}</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="bg-purple-50 border-purple-200">
                      ...
                      <CardContent>
                        <pre className="text-sm text-purple-900 whitespace-pre-wrap font-sans">{note.plan}</pre>
                      </CardContent>
                    </Card>
                  </div>
                </CardContent>
